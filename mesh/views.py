...
class CloudDashboardView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        with connection.cursor() as cursor:
            cursor.execute("select id, event, data_co, data_o2, data_ch4, data_temp, data_humid, volt, created from cloud_mesh_data where device_name = 'xenon1' and created > '2019-12-20' order by created asc")
            sensor1_meshes = cursor.fetchall()

        with connection.cursor() as cursor:
            cursor.execute("select id, event, data_co, data_o2, data_ch4, data_temp, data_humid, volt, created from cloud_mesh_data where device_name = 'xenon2' and created > '2019-12-20' order by created asc")
            sensor2_meshes = cursor.fetchall()

        with connection.cursor() as cursor:
            cursor.execute(
                "select id, event, data_co, data_o2, data_ch4, data_temp, data_humid, volt, created from cloud_mesh_data where device_name = 'xenon3' and created > '2019-12-20' order by created asc")
        ...
        df_sensor1 = df_sensor1[df_sensor1['datetime']>'2019-12-20 10:00']
        df_sensor2 = df_sensor2[df_sensor2['datetime']>'2019-12-20 10:00']
        df_sensor3 = df_sensor3[df_sensor3['datetime']>'2019-12-20 10:00']
...
class MapTestview(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        data = CatM1SensorDataMdodelNew.objects.order_by('-created').values('id', 'device_name', 'created')[:10]
        locs = CatM1LocationMdodelNew.objects.order_by('-created').values('id', 'device_name', 'latitude', 'longitude', 'created')[:10]
